...
export class TileManager extends Evented {
    ...
    constructor(id: string, options: SourceSpecification | CanvasSourceSpecification, dispatcher: Dispatcher) {
        ...
        // `volatile` is a custom (non-standard) vector source option: when set, this source skips the
        // off-screen tile cache entirely so its tiles are never retained once they leave the viewport.
        this._volatile = options.type === 'vector' && (options as any).volatile === true;
    ...
    onAdd(map: Map) {
        ...
        // A volatile source must never retain off-screen tiles; force its cache to zero after the
        // map-level defaults above (which would otherwise re-enable caching).
        if (this._volatile) {
